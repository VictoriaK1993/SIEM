...
public class CsvReader_Labeled_AbnormalSeed2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			NoSeedAttribute = new CsvReader("C:\\Francia\\Baseline1\\baseline1_LabeledNorm.csv");
			SeededAttribute = new CsvReader("C:\\Francia\\\\Baseline1\\sampleAbnormal.csv");
			...
			int recNum = 2;
			while (NoSeedAttribute.readRecord())
			{
				...
				getValues(1);
				setValues();
				if ((insert) && (SeededAttribute.readRecord())){
					//seed the file with abnormal values
					getValues(2);
					setValues();
				}
	...
	static void getValues(int whichFile)
	{
		try {
			
			if (whichFile==1)
				attribute = NoSeedAttribute;
			else attribute = SeededAttribute;
